...
class CustomSubset(Subset):
    ...
    def __init__(self, dataset, indices, subset_transform=None):
        ...
        self.n = dataset.n
        self.newindices = dataset.indices
	
    def __getitem__(self, idx):
        a, b, c, d = self.dataset[self.indices[idx]]
...
def create_dataloader(path,
    ...
    train_indices, test_indices = train_test_split(indices, test_size=0.99)
    customdataset = CustomSubset(dataset, train_indices)
    print(f"original dataset length{len(dataset)},new dataset length{len(customdataset)}")
    batch_size = min(batch_size, len(dataset))
    nd = torch.cuda.device_count()  # number of CUDA devices
    nw = min([os.cpu_count() // max(nd, 1), batch_size if batch_size > 1 else 0, workers])  # number of workers
    sampler = None if rank == -1 else distributed.DistributedSampler(dataset, shuffle=shuffle)
    loader = DataLoader if image_weights else InfiniteDataLoader  # only DataLoader allows for attribute updates
    return loader(customdataset,
                  batch_size=batch_size,
                  shuffle=shuffle and sampler is None,
                  num_workers=nw,
                  sampler=sampler,
                  pin_memory=True,
                  collate_fn=LoadImagesAndLabels.collate_fn4 if quad else LoadImagesAndLabels.collate_fn), customdataset
...
def img2label_paths(img_paths):
    ...
    sa, sb = os.sep + 'num1' + os.sep, os.sep + 'num1' + os.sep  # /images/, /labels/ substrings
    path = [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
    return path
